...
- 同时读取已有的虚拟同事意见工件，重点关注：
  - 交付对象是谁
  - 哪些背景、约束、风险需要补进 PRD
  - 哪些结论只是建议，不能覆盖原型事实
- 本 skill 里的虚拟同事触发节点：
  - 进入本 skill 前，先讨论交付范围、实现约束、缺失信息
  - PRD 初稿准备交付前，再触发一次建议
  - 如果处于高频陪跑模式，PRD 初稿生成后可再做一次复盘
- 虚拟同事负责补背景和风险提醒；正式 PRD 仍由本 skill 负责产出。
...
- 判断是否已有关于“交付范围、实现约束、待补充背景”的前置讨论
- 把这些结论作为 PRD 的背景输入，而不是直接当成已确认需求

这一步的作用是减少交付遗漏，不是让虚拟同事替你写正文。
...
- 如果当前项目启用了虚拟同事，说明这份 PRD 可以作为后续虚拟同事讨论和正式评审的输入，但不要在本 skill 内生成虚拟同事纪要。
