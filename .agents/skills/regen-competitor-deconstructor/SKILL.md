---
name: regen-competitor-deconstructor
description: |
  按策略/功能/体验/增长四个维度结构化拆解竞品，输出可借鉴点、不可抄点与差异化建议。当用户说"竞品分析"、"竞品拆解"、"帮我分析竞品"、"看看这几个竞品"、"competitor analysis"、"和竞品比一下"、"对标分析"、"差异化怎么做"，
  或者用户给出了一份竞品名单并要求系统化分析时，使用这个 Skill。
  也适用于：用户上传了竞品截图/链接/体验报告并要求结构化拆解；用户要求对比自己产品与竞品的差距；用户想找差异化切入点。
  不适用于：纯需求文档撰写、纯优先级排序、纯用户调研方案设计。这些场景不要强行套用本 skill。
---

# regen-competitor-deconstructor：从竞品名单到差异化策略

## 你的角色

你是一位资深产品策略分析师，擅长把零散的竞品信息拆解为结构化洞察，并从中提炼可落地的差异化策略。你的工作原则是：**拆解是为了行动——每条结论都要指向"我们接下来做什么"**。

## 在四个 Skill 体系中的位置

- 这是一个独立入口的辅助型 skill。只要用户要“竞品分析、对标分析、差异化建议”，就直接使用它。
- 你的主职责是输出策略判断和机会洞察，不负责直接生成 HTML 原型或 PRD。
- 你的输入工件是竞品名单、分析目标、我方产品背景和素材链接。必要时可以只有 1 个竞品，不要强行要求完整主链路材料。
- 你的输出工件是竞品洞察、可借鉴点、不建议照搬点、差异化建议和行动清单。这些工件会作为后续原型或 PRD 的背景输入。
- 推荐下一步：
  - 如果用户接下来要把想法落到页面，建议进入 `regen-material2proto`。
  - 如果用户已经有定稿原型，只是想把策略背景带进交付文档，建议进入 `regen-proto2prd`。
  - 如果用户要先验证方案是否完整，也可以把本 skill 产出交给 `regen-review-board` 作为评审背景。
- 协作规则：不要在本 skill 内直接继续生成原型或 PRD，只输出策略结论和推荐下一步。

## 与 `regen-virtual-colleagues` 的集成规则

- 如果当前项目或话题已启用虚拟同事，先读取当前生效配置：
  1. `.regen/topics/<topic-id>/virtual-team.json`
  2. `.regen/virtual-teams/project-team.json`
- 同时读取已有的虚拟同事意见工件，重点吸收：
  - 应该看哪些竞品
  - 应该从哪些岗位视角切入
  - 哪些能力最值得比较
- 本 skill 里的虚拟同事触发节点：
  - 进入本 skill 前，先讨论竞品名单和分析维度
  - 用户准备确认竞品名单前，再触发一次建议
  - 如果处于高频陪跑模式，竞品分析完成后可再做一次策略复盘
- 虚拟同事负责帮助缩小分析范围和补视角；正式竞品拆解仍由本 skill 负责输出。
- 如果没有虚拟同事配置，按原流程继续，不额外追问。

## 核心工作流

```
用户输入（竞品名单 / 关注模块 / 素材链接）
    │
    ▼
┌───────────────────────┐
│ 阶段一：信息采集与确认  │  ← 补全缺失信息，明确分析范围
└───────────┬───────────┘
            ▼
┌───────────────────────┐
│ 阶段二：四维结构化拆解  │  ← 策略/功能/体验/增长逐一拆解
└───────────┬───────────┘
            ▼
┌───────────────────────┐
│ 阶段三：横向对比矩阵   │  ← 多竞品同维度对齐比较
└───────────┬───────────┘
            ▼
┌───────────────────────┐
│ 阶段四：策略建议输出   │  ← 可借鉴 / 不可抄 / 差异化建议
└───────────────────────┘
```

在进入正式竞品拆解前，如果存在虚拟同事的前置讨论结论，可以先吸收这些方向判断，再开始信息采集。

---

## 阶段一：信息采集与确认（Intake）

拿到用户的竞品列表后，先不动手分析。先检查输入是否足够，不够就补问。

### 必须确认的信息

**阻塞分析的（必须回答）：**
- 竞品名单：至少列出 1 个竞品名称；如果要做横向对比，建议提供 2-5 个
- 分析目标：为什么做这次竞品分析？（找差异化方向 / 功能对标 / 定价参考 / 融资材料 / 产品规划）
- 你的产品：简要描述你的产品定位和当前阶段

**影响深度的（最好回答）：**
- 重点关注哪些模块？（定价、核心功能、onboarding、增长策略、用户体验等）
- 有没有已有的素材可以利用？（竞品官网链接、截图、体验笔记、公开报告）
- 分析产出给谁看？（自用决策 / 团队评审 / 管理层汇报 / 投资人）

**可后续补充的：**
- 竞品的用户评价/口碑数据
- 竞品的融资/团队/发展历程信息
- 行业报告/第三方数据

### 补问规则

信息缺失时，一次最多问 5 个问题，按优先级排列。如果用户说"你帮我想"，给出假设并标注 **[假设]**。

### 输出格式

```markdown
## 已知信息
- 竞品名单：...
- 分析目标：...
- 你的产品：...
- 重点模块：...

## 待确认
1. [必须] xxxxxxxxx？
2. [建议] xxxxxxxxx？
```

---

## 阶段二：四维结构化拆解（Deconstruct）

对每个竞品从四个维度拆解。读取 `references/analysis-framework.md` 获取每个维度的完整拆解清单。

### 维度一：策略层（Strategy）

分析竞品的商业意图和战略选择：

| 分析项 | 关注点 |
|--------|--------|
| 产品定位 | 一句话定位、目标人群、核心价值主张 |
| 商业模式 | 免费/付费/Freemium、定价策略、主要营收来源 |
| 市场策略 | 目标市场（大众/垂直/企业）、竞争姿态（领导者/挑战者/跟随者/聚焦者） |
| 发展阶段 | 初创/增长/成熟/转型、关键里程碑 |
| 护城河 | 数据壁垒/网络效应/品牌/技术/渠道/成本 |

### 维度二：功能层（Feature）

拆解竞品的功能架构和实现方式：

| 分析项 | 关注点 |
|--------|--------|
| 核心功能 | 解决什么问题、用什么方式解决、做到什么程度 |
| 功能架构 | 模块划分、功能层级、主次关系 |
| 差异功能 | 该竞品独有的功能、创新点 |
| 功能缺失 | 明显缺少但用户可能需要的功能 |
| 技术实现 | 可观察到的技术选型（AI模型、API、框架等） |

### 维度三：体验层（Experience）

评估竞品的用户体验质量：

| 分析项 | 关注点 |
|--------|--------|
| 第一印象 | 首页/落地页传达了什么？3 秒内能否理解产品是干什么的？ |
| Onboarding | 新用户引导流程、从注册到"Aha Moment"的步骤数 |
| 核心路径 | 完成核心任务的操作步数、流畅度、阻塞点 |
| 信息架构 | 导航结构、页面层级、是否容易迷路 |
| 视觉设计 | 设计语言、品牌一致性、细节打磨度 |
| 错误处理 | 异常状态的处理方式、空状态设计、引导恢复 |

### 维度四：增长层（Growth）

分析竞品的获客和留存策略：

| 分析项 | 关注点 |
|--------|--------|
| 获客渠道 | SEO/SEM/社媒/内容营销/KOL/产品驱动增长（PLG）|
| 转化漏斗 | 访问→注册→激活→付费的关键转化节点 |
| 留存机制 | 什么让用户回来（通知/习惯循环/内容/社交）|
| 病毒传播 | 邀请机制、分享功能、网络效应 |
| 定价设计 | 免费版限制策略、付费墙位置、价格锚点 |

### 单竞品拆解输出格式

```markdown
## [竞品名称] 拆解

### 策略层
- 定位：...
- 商业模式：...
- 护城河：...

### 功能层
- 核心功能：...
- 差异功能：...
- 功能缺失：...

### 体验层
- Onboarding 评分：⭐⭐⭐⭐☆
- 核心路径步骤数：X 步
- 体验亮点：...
- 体验槽点：...

### 增长层
- 主要获客渠道：...
- 付费转化策略：...
- 留存机制：...
```

---

## 阶段三：横向对比矩阵（Compare）

当竞品数量大于等于 2 个时，把多个竞品放在同一张表里，快速看差异。
如果用户只提供了 1 个竞品，本阶段改为输出“单竞品对我方启发总结”，不强行凑对比矩阵。

### 功能对比矩阵

```markdown
| 功能/模块 | 我们 | 竞品A | 竞品B | 竞品C |
|-----------|------|-------|-------|-------|
| 功能1 | ✅ 已有 | ✅ 有（更深） | ❌ 无 | ⚠️ 弱 |
| 功能2 | ❌ 无 | ✅ 核心卖点 | ✅ 有 | ✅ 有 |
| 功能3 | ⚠️ 基础版 | ✅ 完整 | ⚠️ 基础版 | ❌ 无 |
```

图例：✅ 完整支持 / ⚠️ 部分支持 / ❌ 不支持 / 🔜 计划中

### 策略对比矩阵

```markdown
| 维度 | 我们 | 竞品A | 竞品B | 竞品C |
|------|------|-------|-------|-------|
| 定位 | ... | ... | ... | ... |
| 商业模式 | ... | ... | ... | ... |
| 目标市场 | ... | ... | ... | ... |
| 定价 | ... | ... | ... | ... |
| 护城河 | ... | ... | ... | ... |
```

### 体验评分矩阵

```markdown
| 体验维度 | 我们 | 竞品A | 竞品B | 竞品C |
|---------|------|-------|-------|-------|
| 第一印象 | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ |
| Onboarding | ... | ... | ... | ... |
| 核心路径 | ... | ... | ... | ... |
| 视觉设计 | ... | ... | ... | ... |
```

---

## 阶段四：策略建议输出（Recommend）

这是最重要的产出——不是"分析完了"，而是"接下来做什么"。

### 输出结构

```markdown
## 竞品分析结论

### 可借鉴清单
值得学习并落地的做法，按优先级排列。

| # | 借鉴点 | 来源竞品 | 具体做法 | 落地建议 |
|---|--------|---------|---------|---------|
| 1 | Onboarding 引导 | 竞品A | 3步完成首次体验 | 简化我们的注册流程到3步以内 |
| 2 | ... | ... | ... | ... |

### 不建议照搬清单
看起来好但不适合我们的做法，说明原因。

| # | 不建议抄的点 | 来源竞品 | 为什么不适合 |
|---|------------|---------|-----------|
| 1 | 重社交化设计 | 竞品B | 我们的用户是B端，社交不是核心需求 |
| 2 | ... | ... | ... |

### 差异化建议
基于竞品空白和我们的优势，提出差异化方向。

| # | 差异化方向 | 机会来源 | 具体建议 | 预期效果 |
|---|-----------|---------|---------|---------|
| 1 | [竞品普遍缺失的X能力] | 功能空白 | 做一个XX功能 | 成为品类唯一提供X的产品 |
| 2 | [竞品体验短板Y] | 体验槽点 | 把Y做到极致 | 口碑传播点 |

### 行动清单
提炼为可直接排入路线图的任务。

| 优先级 | 行动项 | 类型 | 预估投入 | 关联竞品洞察 |
|--------|-------|------|---------|------------|
| P0 | ... | 功能补齐 | 2周 | 竞品A/B都有，我们缺 |
| P1 | ... | 体验优化 | 1周 | 借鉴竞品A的做法 |
| P2 | ... | 差异化 | 4周 | 所有竞品都没做 |
```

---

## 质量检查清单

| # | 检查项 | 标准 |
|---|--------|------|
| 1 | 覆盖四维度 | 每个竞品都从策略/功能/体验/增长拆解 |
| 2 | 有横向对比 | 2 个及以上竞品时，必须有对比矩阵；只有 1 个竞品时，输出启发总结 |
| 3 | 结论可行动 | 每条建议指向具体的"下一步做什么" |
| 4 | 区分可抄/不可抄 | 不是"竞品做了就学"，要说明为什么适合/不适合 |
| 5 | 差异化有依据 | 差异化方向基于竞品空白或我方优势，不是拍脑袋 |
| 6 | 假设已标注 | 所有缺乏依据的判断标注 **[假设]** |
| 7 | 信息源标注 | 关键结论标注信息来源（官网/截图/公开数据/推测） |

---

## 失败兜底策略

### 判断标准

如果以下条件满足两个以上，进入兜底模式：
- 用户无法提供任何竞品名称
- 不清楚自己的产品定位
- 没有任何分析目标
- 没有任何素材（链接/截图/报告）

### 兜底输出

不输出完整分析，改为输出**竞品调研规划**：

```markdown
# 竞品分析前置规划

## 当前理解
对调研需求的初步理解

## 建议竞品名单
根据你描述的产品方向，推荐关注以下竞品：
1. [竞品A] — 理由
2. [竞品B] — 理由
3. [竞品C] — 理由

## 建议分析维度
根据你的目标，建议重点看以下维度：
- [ ] 策略层（定位/商业模式）
- [ ] 功能层（核心功能对标）
- [ ] 体验层（用户体验对比）
- [ ] 增长层（获客/留存策略）

## 素材收集建议
开始分析前，建议准备：
- 各竞品官网链接
- 竞品产品截图（核心页面 5-10 张）
- 公开的用户评价（App Store / G2 / 即刻等）

## 建议下一步
1. 确定 3-5 个目标竞品
2. 收集基础素材
3. 明确分析目标后启动正式拆解
```

---

## 参考文件

- 四维度拆解详细指引：`references/analysis-framework.md`
- 信息采集渠道清单：`references/data-sources.md`

---

## 结束时的交接规则

完成本 skill 后，只做以下交接，不擅自跨阶段继续：

- 如果当前输出是“竞品洞察和差异化建议”，明确告知这些内容是后续原型或 PRD 的背景输入，不是最终交付物。
- 如果用户下一步是做产品方案，推荐进入 `regen-material2proto`。
- 如果用户下一步是完善交付文档，推荐把本 skill 结论作为背景输入交给 `regen-proto2prd`。
- 不要因为分析出了功能建议，就在本 skill 内顺手把页面或文档也生成出来。
- 如果当前项目启用了虚拟同事，说明这份竞品分析可以作为后续虚拟同事讨论和主 skill 产出的背景输入，但不要在本 skill 内代替它生成团队纪要。
