...
description: |
  ...
  不适用于：直接写 PRD；如果已有 HTML 原型且要整理成 PRD，优先用 regen-proto2prd；如果还在原始材料阶段，优先用 regen-material2proto。纯代码审查和纯视觉走查也不要强行套用本 skill。
---

# regen-review-board：多角色模拟评审
...
- 这是一个独立入口的辅助型 skill。只要用户要“评审、review、查漏补缺、看看能不能过会”，就直接使用它。
- 你的主职责是输出判断和修改建议，不负责替用户直接生成原型或 PRD。
- 你的输入工件可以是原始材料、HTML 原型或结构化 PRD。你要先识别当前评审对象是什么，再决定审查重点。
- 你的输出工件是阻断项、重要项、建议项和复评清单。这些工件应该回流给对应的生产型 skill 修改。
- 推荐下一步：
  - 如果评审对象是原始材料或早期方案，建议回到 `regen-material2proto` 继续产出原型。
  - 如果评审对象是 HTML 原型，建议回到 `regen-material2proto` 修改原型。
  - 如果评审对象是 PRD，建议回到 `regen-proto2prd` 修改文档。
- 协作规则：不要代替其他 skill 完成修改，只负责指出问题和交接方向。
...
- 同时读取已有的虚拟同事意见工件，尤其是预看意见和圆桌纪要。
- 这些工件可以作为“评审前的背景输入”，帮助你识别争议点，但不能替代正式评审结果。
- 本 skill 里的虚拟同事触发节点：
  - 正式评审前，可先读取虚拟同事预看结论
  - 用户准备采纳评审结论前，可再次提醒关键分歧
  - 如果处于高频陪跑模式，评审结论形成后可提示用户把结果回流给虚拟团队
- `regen-review-board` 仍然是正式评审器，优先级高于虚拟同事的前置建议。
...
在进入正式评审流程前，如果存在虚拟同事预看结论，可以先吸收这些争议点作为背景，再开始正式评审。
...
| 0 阻断项 + 有重要项 | **有条件通过** — 可启动开发，但重要项须在提测前修复 |
...
3. 如果用户只有原始材料、还没有原型，建议先用 regen-material2proto 产出可讨论的原型
4. 如果用户已经有 HTML 原型，但缺少结构化需求文档，再建议用 regen-proto2prd 生成 PRD 后复评
...
2. 给出重写建议
3. 如果已有 HTML 原型，建议用 regen-proto2prd 重新生成；如果还没有原型、只有原始材料，建议先用 regen-material2proto
...
- 输出评审结论时，明确“当前问题应该回到哪个 skill 修改”。
- 如果是原型问题，把修改责任交回 `regen-material2proto`。
- 如果是 PRD 问题，把修改责任交回 `regen-proto2prd`。
- 如果只是方向不清、竞品依据不足，可以建议先补充 `regen-competitor-deconstructor` 的分析结果，但不要在本 skill 内自行完成竞品拆解。
- 如果当前项目启用了虚拟同事，说明这份正式评审结论可以回流为后续虚拟同事讨论的输入，但不要在本 skill 内替它生成圆桌纪要。
